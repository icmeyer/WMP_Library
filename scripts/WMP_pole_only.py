...
class WindowedMultipole(object):
    ...
    def _evaluate(self, E, T, curve_flag=False):
        ...
        sig_s_wmp = sig_s - sig_s_curve
        sig_a_wmp = sig_a - sig_a_curve
        sig_f_wmp = sig_f - sig_f_curve

        if curve_flag:
            return sig_s_wmp, sig_s_curve, \
                   sig_a_wmp, sig_a_curve, \
                   sig_f_wmp, sig_f_curve
        else:
            return sig_s, sig_a, sig_f

    def __call__(self, E, T, curve_flag=False):
        """Compute scattering, absorption, and fission cross sections.
        ...
        T : Real
            Temperature of the target in K.
        curve_flag : bool
            Return separate contributions of curve fit and WMP residues
        ...
        fun = np.vectorize(lambda x: self._evaluate(x, T, curve_flag))
        return fun(E)
